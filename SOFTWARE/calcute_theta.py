...
# 加载保存的模型和预处理对象
# ----------------------------
model = joblib.load('./CODE/best_mlp_model_theta.joblib')
scaler = joblib.load('./CODE/scaler_theta.joblib')
imputer = joblib.load('./CODE/imputer_theta.joblib')
...
def predict_THETA(input_features):
    df_input = pd.DataFrame([input_features])

    # X_input = df_input[features]
    X_input_imputed = pd.DataFrame(imputer.transform(df_input), columns=features)
    X_input_scaled = scaler.transform(X_input_imputed)
    Theta_pred = model.predict(X_input_scaled)
    return Theta_pred[0]
